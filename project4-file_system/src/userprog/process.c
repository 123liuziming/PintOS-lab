...
start_process (void *file_name_)
{
  ...
  char* file_name;
  char* name_str;
  const int params_len = strlen(file_name_);
  full_str = (char*) malloc(params_len + 1);
  name_str = (char*) malloc(params_len + 1);
  file_name = (char*) malloc(params_len + 1);
  strlcpy (name_str, file_name_, PGSIZE);
  strlcpy (file_name, file_name_, PGSIZE);
  strlcpy (full_str, file_name_, PGSIZE);
  ...
  char* save_ptr;
  // 计算args[i]对应的在栈中的地址
  int* stack_addr[MAX_CMD_ARGS_COUNT];
  ...
  for (token = strtok_r (full_str, " ", &save_ptr); token != NULL; token = strtok_r (NULL, " ", &save_ptr)) {
    ...
    if_.esp -= token_len;
    memcpy(if_.esp, token, token_len);
    stack_addr[argc] = if_.esp;
    ++argc;
  }
  // 对齐
  while ((int)if_.esp % 4) {
